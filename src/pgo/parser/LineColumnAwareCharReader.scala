...
/** A generic Char reader wrapper, which counts line and column numbers within
  ...
  *
  * TLA+ parsing assumes this wrapper is available, because parsing of /\ and \/
  * relies on knowing indentation.
  *
  * @param underlying
  *   the underlying Reader to wrap
  * @param line
  *   the current line. Either leave at its default of 0, or set this to the
  *   line from which you want to start counting.
  * @param column
  *   the current column. See above.
  */
final class LineColumnAwareCharReader(
