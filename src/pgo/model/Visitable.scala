    ...
    def reduceSubNode(node: Any): C =
      node match {
        case visitable: Visitable =>
          val subNodeResults =
            visitable.productIterator.map(reduceSubNode).toSeq
          fn.applyOrElse(
        ...
        case iterable: Iterable[_] =>
          iterable.iterator.map(reduceSubNode).foldLeft(empty)(reducer)
        case product: Product =>
          product.productIterator.map(reduceSubNode).foldLeft(empty)(reducer)
        case _ => empty
      }
    reduceSubNode(this)
  }
