...
private final case class DescriptionStringPart(str: String)
    extends DescriptionPart
private final case class DescriptionEmbedPart(embed: Any)
    extends DescriptionPart

final class Description private (private val parts: View[DescriptionPart])
    extends Description.Describable {
  ...
  def linesIterator: Iterator[String] =
    new AbstractIterator[String] {
      private val partsIterator = parts.iterator.flatMap {
        case DescriptionEmbedPart(embed) =>
          Description.stringToDescriptionParts(embed.toString)
        case part => Iterator.single(part)
      }
      private var atStart =
        true // used to detect the "first line", for ensuring leading line breaks work properly
      ...
      private var indent =
        0 // the indentation to apply after a new line, if the following line has any contents
      ...
      private def gatherNextLine(): Unit = {
        ...
        if ((lineBuilder.nonEmpty || hasNewLine) && (nextLine eq null)) {
          hasNewLine =
            false // if a trailing new line is output, make sure we stop there
          nextLine = lineBuilder.result()
          lineBuilder.clear()
        }
      }
...
object Description {
  ...
  final implicit class DescribableNumber[T: Numeric](num: T)
      extends Describable {
  ...
  extension (descList: Iterable[Description])
    ...
    def separateBy(desc: Description): Description =
      new Description(View.fromIteratorProvider { () =>
        var first = true
        descList.iterator.flatMap(elem =>
          if (first) {
            first = false
            elem.parts.iterator
