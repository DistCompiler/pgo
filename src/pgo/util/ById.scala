...
/** A wrapper class that compares the wrapped value by "pointer identity" (the
  * JVM equivalent), bypassing whatever value semantics the wrapped value might
  * have.
  */
...
object ById {
  ...
  /** Provides a factory of {{{Map[ById[K],V]]}}} from elements of type
    * {{{(K,V)}}}
    */
  def mapFactory[K <: AnyRef, V]: Factory[(K, V), Map[ById[K], V]] =
    new MapByIdFactory

  private class SetByIdBuilder[V <: AnyRef]
      extends mutable.Builder[V, Set[ById[V]]] {
  ...
  private class SetByIdFactory[V <: AnyRef] extends Factory[V, Set[ById[V]]] {
    ...
    override def newBuilder: mutable.Builder[V, Set[ById[V]]] =
      new SetByIdBuilder
  }

  private class MapByIdBuilder[K <: AnyRef, V]
      extends mutable.Builder[(K, V), Map[ById[K], V]] {
  ...
  private class MapByIdFactory[K <: AnyRef, V]
      extends Factory[(K, V), Map[ById[K], V]] {
    override def fromSpecific(it: IterableOnce[(K, V)]): Map[ById[K], V] =
      it.iterator.map { case (k, v) => ById(k) -> v }.toMap
    override def newBuilder: mutable.Builder[(K, V), Map[ById[K], V]] =
      new MapByIdBuilder
  }
}
