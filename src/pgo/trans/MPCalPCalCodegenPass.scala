...
object MPCalPCalCodegenPass {
  ...
  object MPCalPCalEffectivelyNoProcessesError {
    final case class MPCalEffectivelyNoProcesses(mpcalBlock: MPCalBlock)
        extends PGoError.Error {
      override def sourceLocation: SourceLocation = mpcalBlock.sourceLocation
      override val description: Description =
        d"Modular PlusCal block has effectively no processes"
    }
  }
  ...
  def apply(tlaModule: TLAModule, mpcalBlock: MPCalBlock): PCalAlgorithm = {
    ...
    tlaModule.visit(Visitable.BottomUpFirstStrategy) { case TLAIdentifier(id) =>
      nameCleaner.addKnownName(id)
    }
    block.visit(Visitable.BottomUpFirstStrategy) { case TLAIdentifier(id) =>
      nameCleaner.addKnownName(id)
    }
    ...
    block = locally {
      ...
      /** Transform the statement according to the provided substitutions.
        * Accounts for assignment LHS, but will not work for unusual things like
        ...
        if (substitutions.isEmpty) {
          ...
          stmt.rewrite(Rewritable.TopDownFirstStrategy) {
            case ident @ TLAGeneralIdentifier(_, pfx)
                if substitutions.contains(ById(ident.refersTo)) =>
              assert(pfx.isEmpty)
      ...
      /** Transforms any with-bindings in the provided PCal statement, such that
        * the bound names use fresh names. This is important when expanding
        * mapping macros that contain with-bindings, because expanding the same
        * mapping macro twice might lead to one with-statement in the macro
        * containing itself, leading to a scoping error.
        */
      ...
      /** Transform the statement according to the provided mapping macros. Does
      ...
      def applyMappingMacros(stmt: PCalStatement)(using
        ...
        def updateReads[E <: Rewritable](
          ...
          expr.rewrite(Rewritable.TopDownFirstStrategy) {
            case expr @ MappedRead(mappingCount, ident)
                if !skipMappings && mappingsMap
                  .get(ById(ident.refersTo))
                  .exists(_._1 == mappingCount) =>
              ...
              def translateLhs(expr: TLAExpression): PCalAssignmentLhs =
                expr match {
                  case expr @ TLAGeneralIdentifier(name, Nil) =>
                    PCalAssignmentLhsIdentifier(name).setRefersTo(expr.refersTo)
                  case TLAFunctionCall(fn, arguments) =>
                    PCalAssignmentLhsProjection(translateLhs(fn), arguments)
                  case _ =>
                    !!! // MappedRead matching should ensure exhaustivity of the cases above
                }
                  ...
                  mapping.readBody
                    ...
                    .mapConserve(_.rewrite(Rewritable.BottomUpOnceStrategy) {
                      case ident: TLAGeneralIdentifier
                          if ident.refersTo eq mapping.selfDecl =>
        ...
        val unwrappedStmt = stmt match {
          case stmt: PCalAssignment =>
            ...
            val withReads @ PCalAssignment(List(PCalAssignmentPair(lhs, rhs))) =
              updateReads(stmt)
            findRef(lhs)
              .flatMap {
                case ident if mappingsMap.contains(ById(ident.refersTo)) =>
                  val (mappingCount, mapping) =
                    mappingsMap(ById(ident.refersTo))
            ...
            variables match {
              ...
              case decl :: restDecls =>
                ...
                val localSubstitutions
                    : Map[ById[RefersTo.HasReferences], DefinitionOne] =
                  if (subbedDecl ne decl) Map(ById(decl) -> subbedDecl)
                  else Map.empty
          ...
          case PCalExtensionStatement(call @ MPCalCall(_, arguments)) =>
            ...
            val byRefArgs = arguments.collect { case Left(ref) =>
              ref
            }
            val byValueArgs = arguments.collect { case Right(expr) =>
              updateReads(expr)
            }
            val refSignature = MPCalProcedureSignature(
              proc = ById(mpcalProcedure),
              refArgs = byRefArgs.map { ref =>
                ById(ref.refersTo) -> mappingsMap
            ...
            val cacheRecord = mpcalProcedureCache.getOrElseUpdate(
              ...
              Right {
                val freshName =
                  TLAIdentifier(nameCleaner.cleanName(mpcalProcedure.name.id))
                ...
                val recursionFixList =
                  mutable.ListBuffer[PCalProcedure => Unit]()
                mpcalProcedureCache(refSignature) =
                  Left((freshName, recursionFixList))

                val refParams = mpcalProcedure.params.collect {
                  case ref: MPCalRefParam => ref
                }
                assert(refParams.size == byRefArgs.size)
                val valParams = mpcalProcedure.params.collect {
                  case v: MPCalValParam => v
                }
      ...
      val repairSubstitutions =
        mutable.HashMap[ById[RefersTo.HasReferences], DefinitionOne]()

      // TODO: single-process MPCal
      val rewritten = block.rewrite(Rewritable.TopDownFirstStrategy) {
        case instance @ MPCalInstance(
              selfDecl,
              fairness,
              archetypeName,
              arguments,
          ...
          val substitutionsBuilder =
            mutable.Buffer.empty[(RefersTo.HasReferences, DefinitionOne)]
          val variables = archetype.variables.iterator
            .map { v =>
          ...
          (arguments.iterator zip archetype.params).foreach {
            ...
            case Right(expr) -> param =>
              ...
              variables += PCalVariableDeclarationValue(
                TLAIdentifier(stateVarNameCleaner.cleanName(param.name.id)),
                expr.rewrite(Rewritable.TopDownFirstStrategy) {
                  case ident @ TLAGeneralIdentifier(_, _)
          ...
          val mappingsMap = mappings.iterator
            .map {
      ...
      val pcalAlgorithm = rewritten.copy(
        pcalProcedures =
          rewritten.pcalProcedures ++ generatedPCalProcedures.result(),
        mappingMacros = Nil,
        mpcalProcedures = Nil,
        instances = Nil,
    ...
    block = block.rewrite(Rewritable.BottomUpOnceStrategy) {
      case PCalAssignment(List(PCalAssignmentPair(lhs, rhs))) =>
        ...
        val substitutionKeys =
          findSubstitutionKeys(lhs, mutable.ListBuffer.empty)
        val wrappedRhs =
          if (substitutionKeys.isEmpty) {
            rhs
          } else {
            TLAFunctionSubstitution(
              source = TLAGeneralIdentifier(ident.identifier, Nil)
    ...
    block = locally {
      ...
      block.rewrite(Rewritable.TopDownFirstStrategy) {
        case labeledStmts @ PCalLabeledStatements(label, body) =>
          ...
          var assignmentCountsStmtMap =
            Map.empty[ById[PCalStatement], Map[ById[DefinitionOne], Int]]
          var assignmentCountsMap =
              ...
              ById(stmt), {
                ...
                stmt.visit(Visitable.TopDownFirstStrategy) {
                  case stmt: PCalStatement
                      if assignmentCountsStmtMap.contains(ById(stmt)) =>
                  ...
                  case PCalEither(cases) =>
                    result +++= cases.iterator
                      .map(assignmentCounts)
                      .reduce(_ ||| _)
                  case PCalAssignment(
                        List(
                          PCalAssignmentPair(
          ...
          object TailJumpStmts {
            ...
            def unapply(
              ...
              stmts match {
                ...
                case PCalCall(_, _) :: (((PCalReturn() | PCalGoto(_)) :: Nil) |
                    Nil) =>
                  Some(stmts)
                case PCalExtensionStatement(
                      MPCalCall(_, _),
          ...
          def impl(
            ...
            /** Perform substitutions, _specifically avoiding assignment LHS_!
              * Fundamentally, these subs are for temporary with-bindings, and
              * they should never change an assignment target.
              *
              * "Adding the LHS case in the hope it would fix something, only to
              * break something" count of shame: 1, pre-comment
              */
                ...
                hd match {
                  ...
                  case PCalWith(bindings, body) =>
                    ...
                    val renamedBindings = bindings.map { decl =>
                      ...
                      nextSubstitutions =
                        nextSubstitutions.updated(ById(decl), result)
                      result
                    }
                    List(
                      PCalWith(
