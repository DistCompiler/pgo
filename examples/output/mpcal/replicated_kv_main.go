...
func init() {
	...
	for _, replica := range replicated_kv.ReplicaSet() {
	    //fmt.Printf("Replica(%d)", replica)
	    configuration[fmt.Sprintf("Replica(%d)", replica)] = fmt.Sprintf("127.0.0.1:%d", 6000 + replica)
	}
	for _, client := range replicated_kv.ClientSet() {
		//fmt.Printf("Client(%d)", client)
    	configuration[fmt.Sprintf("Client(%d)", client)] = fmt.Sprintf("127.0.0.1:%d", 6000 + client)
    }
	...
	coordinator := configuration["Replica(0)"]
...
func main() {
	...
	if role == "Client" {
		...
		mkBytes := func() string {
		    result := ""
		    for i := 0; i < 32; i++ {
		        result += string(alphanum[rand.Intn(len(alphanum))])
		    }
		    return result
		}
		for i := 0; i < 100; i++ {
		    if rand.Intn(1) == 0 {
		        commands = append(commands, Get{ key: mkBytes() })
		    } else {
		        commands = append(commands, Put{ key: mkBytes(), value: mkBytes() + mkBytes() })
		    }
		}
