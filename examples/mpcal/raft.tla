...
--mpcal Raft {
    ...
    \* Stable FIFOChannels, guarantees (eventual) termination and no deadlock
    ...
    \* Unstable FIFOChannels, ensures correct algorithm behavior in the face of network issues
    mapping macro UnstableFIFOChannel {
        read {
            await Len($variable) > 0;
            with (msg = Head($variable)) {
                $variable := Tail($variable);
        ...
        write {
            await Len($variable) < BUFFER_SIZE;
            either { yield $variable; } \* messages may be dropped
    ...
    variable currentTerm = 0,
             ...
             votes = [t \in 0..Terms |-> {}],
             msg; {
MainLoop:while (Len(log) < Slots /\ currentTerm < Terms) {
...
--algorithm Raft {
    variables mailboxes = [id \in Servers |-> <<>>], timeoutRead, networkWrite, networkWrite0, networkWrite1, valuesRead, valuesWrite, appliedWrite, appliedWrite0, networkWrite2, valuesWrite0, appliedWrite1, networkWrite3, networkRead, networkWrite4, networkWrite5, networkWrite6, ifResult, appliedWrite2, appliedWrite3, networkWrite7, networkWrite8, ifResult0, networkWrite9, networkWrite10, networkWrite11, networkWrite12, appliedWrite4, networkWrite13, valuesWrite1, appliedWrite5;
    define {
        Servers == (1) .. (N)
        RequestVote == 0
        RequestVoteResponse == 1
    ...
    variables appliedLocal = [s \in (0) .. (Slots) |-> {}], valuesLocal = <<1, 2, 3, 4, 5>>, timeoutLocal = TRUE, currentTerm = 0, votedFor = Nil, log = <<>>, state = Follower, commitIndex = 0, lastApplied = 0, nextIndex, matchIndex, iterator, votes = [t \in (0) .. (Terms) |-> {}], msg;
    {
        MainLoop:
            if (((Len(log)) < (Slots)) /\ ((currentTerm) < (Terms))) {
                TimeoutCheck:
                    either {
                        timeoutRead := FALSE;
                    } or {
...
VARIABLES mailboxes, timeoutRead, networkWrite, networkWrite0, networkWrite1, 
          valuesRead, valuesWrite, appliedWrite, appliedWrite0, networkWrite2, 
          valuesWrite0, appliedWrite1, networkWrite3, networkRead, 
          networkWrite4, networkWrite5, networkWrite6, ifResult, 
          appliedWrite2, appliedWrite3, networkWrite7, networkWrite8, 
          ifResult0, networkWrite9, networkWrite10, networkWrite11, 
          networkWrite12, appliedWrite4, networkWrite13, valuesWrite1, 
          appliedWrite5, pc
...
vars == << mailboxes, timeoutRead, networkWrite, networkWrite0, networkWrite1, 
           valuesRead, valuesWrite, appliedWrite, appliedWrite0, 
           networkWrite2, valuesWrite0, appliedWrite1, networkWrite3, 
           networkRead, networkWrite4, networkWrite5, networkWrite6, ifResult, 
           appliedWrite2, appliedWrite3, networkWrite7, networkWrite8, 
           ifResult0, networkWrite9, networkWrite10, networkWrite11, 
           networkWrite12, appliedWrite4, networkWrite13, valuesWrite1, 
           appliedWrite5, pc, appliedLocal, valuesLocal, timeoutLocal, 
...
MainLoop(self) == /\ pc[self] = "MainLoop"
                  /\ IF ((Len(log[self])) < (Slots)) /\ ((currentTerm[self]) < (Terms))
                        THEN /\ pc' = [pc EXCEPT ![self] = "TimeoutCheck"]
                             /\ UNCHANGED << mailboxes, networkWrite13, 
                                             valuesWrite1, appliedWrite5, 
                  ...
                  /\ UNCHANGED << timeoutRead, networkWrite, networkWrite0, 
                                  ...
                                  valuesWrite0, appliedWrite1, networkWrite3, 
                                  networkRead, networkWrite4, networkWrite5, 
                                  networkWrite6, ifResult, appliedWrite2, 
                                  appliedWrite3, networkWrite7, networkWrite8, 
                                  ifResult0, networkWrite9, networkWrite10, 
                                  networkWrite11, networkWrite12, 
                                  appliedWrite4, timeoutLocal, currentTerm, 
...
TimeoutCheck(self) == /\ pc[self] = "TimeoutCheck"
                      ...
                      /\ UNCHANGED << networkWrite, networkWrite0, valuesRead, 
                                      ...
                                      networkWrite2, valuesWrite0, 
                                      appliedWrite1, networkWrite3, 
                                      networkRead, networkWrite4, 
                                      networkWrite5, networkWrite6, ifResult, 
                                      appliedWrite2, appliedWrite3, 
                                      networkWrite7, networkWrite8, ifResult0, 
                                      networkWrite9, networkWrite10, 
                                      networkWrite11, networkWrite12, 
                                      appliedWrite4, networkWrite13, 
                                      valuesWrite1, appliedWrite5, 
...
LeaderCheck(self) == /\ pc[self] = "LeaderCheck"
                     ...
                     /\ UNCHANGED << timeoutRead, networkWrite, networkWrite0, 
                                     ...
                                     networkWrite4, networkWrite5, 
                                     networkWrite6, ifResult, appliedWrite2, 
                                     appliedWrite3, networkWrite7, 
                                     networkWrite8, ifResult0, networkWrite9, 
                                     networkWrite10, networkWrite11, 
                                     networkWrite12, appliedWrite4, 
                                     networkWrite13, valuesWrite1, 
                                     appliedWrite5, timeoutLocal, currentTerm, 
...
SendAppendEntries(self) == /\ pc[self] = "SendAppendEntries"
                           ...
                           /\ UNCHANGED << timeoutRead, networkWrite0, 
                                           ...
                                           appliedWrite0, valuesWrite0, 
                                           appliedWrite1, networkWrite3, 
                                           networkRead, networkWrite4, 
                                           networkWrite5, networkWrite6, 
                                           ifResult, appliedWrite2, 
                                           appliedWrite3, networkWrite7, 
                                           networkWrite8, ifResult0, 
                                           networkWrite9, networkWrite10, 
                                           networkWrite11, networkWrite12, 
                                           appliedWrite4, networkWrite13, 
                                           valuesWrite1, appliedWrite5, 
...
RecvMsg(self) == /\ pc[self] = "RecvMsg"
                 ...
                 /\ UNCHANGED << timeoutRead, networkWrite0, networkWrite1, 
                                 ...
                                 appliedWrite0, networkWrite2, valuesWrite0, 
                                 appliedWrite1, networkWrite3, networkWrite4, 
                                 networkWrite5, networkWrite6, ifResult, 
                                 appliedWrite2, appliedWrite3, networkWrite7, 
                                 networkWrite8, ifResult0, networkWrite9, 
                                 networkWrite10, networkWrite11, 
                                 networkWrite12, appliedWrite4, networkWrite13, 
                                 valuesWrite1, appliedWrite5, appliedLocal, 
...
CheckMsgTerm(self) == /\ pc[self] = "CheckMsgTerm"
                      ...
                      /\ UNCHANGED << mailboxes, timeoutRead, networkWrite, 
                                      ...
                                      valuesWrite1, appliedWrite5, 
                                      appliedLocal, valuesLocal, timeoutLocal, 
                                      votedFor, log, commitIndex, lastApplied, 
                                    ...
                                    ELSE /\ IF ((((msg[self]).prevIndex) > (0)) /\ ((Len(log[self])) < ((msg[self]).prevIndex))) \/ ((Term(log[self], (msg[self]).prevIndex)) # ((msg[self]).prevTerm))
                                               ...
                                               ELSE /\ IF (Len(log[self])) = ((msg[self]).prevIndex)
                                                          ...
                                                          ELSE /\ networkWrite4' = mailboxes
                                                               ...
                                                               /\ log' = log
                              /\ UNCHANGED << ifResult0, networkWrite9, 
                                              networkWrite10, networkWrite11, 
                                              networkWrite12, appliedWrite4, 
                         ...
                         ELSE /\ IF ((msg[self]).type) = (RequestVote)
                                    ...
                                    ELSE /\ IF ((((msg[self]).type) = (AppendEntriesResponse)) /\ (((msg[self]).term) = (currentTerm[self]))) /\ ((state[self]) = (Leader))
                                               ...
                                               ELSE /\ IF ((((msg[self]).type) = (RequestVoteResponse)) /\ (((msg[self]).term) = (currentTerm[self]))) /\ ((state[self]) = (Candidate))
                                                    ...
                                                    /\ UNCHANGED << ifResult0, 
                                                                    networkWrite9 >>
                              /\ UNCHANGED << networkWrite4, networkWrite5, 
                                              networkWrite6, log >>
                   /\ UNCHANGED << timeoutRead, networkWrite, networkWrite0, 
                                   networkWrite1, valuesRead, valuesWrite, 
                                   appliedWrite, appliedWrite0, networkWrite2, 
                                   ...
                                   appliedWrite3, networkWrite7, networkWrite8, 
                                   networkWrite13, valuesWrite1, appliedWrite5, 
                                   valuesLocal, timeoutLocal, currentTerm, 
...
AEStale(self) == /\ pc[self] = "AEStale"
                 ...
                 /\ UNCHANGED << timeoutRead, networkWrite0, networkWrite1, 
                                 ...
                                 networkWrite4, networkWrite5, networkWrite6, 
...
RVInvalid(self) == /\ pc[self] = "RVInvalid"
                   ...
                   /\ UNCHANGED << timeoutRead, networkWrite0, networkWrite1, 
                                   ...
                                   networkWrite9, networkWrite10, 
                                   networkWrite11, networkWrite12, 
                                   appliedWrite4, networkWrite13, valuesWrite1, 
                                   appliedWrite5, appliedLocal, valuesLocal, 
...
server(self) == MainLoop(self) \/ TimeoutCheck(self) \/ SendReqVotes(self)
                   ...
                   \/ RecvMsg(self) \/ CheckMsgTerm(self)
                   \/ MsgSwitch(self) \/ AEUpdateCommited(self)
                   \/ AEApplyCommitted(self) \/ AEStale(self)
                   \/ AENotMatching(self) \/ AEValid(self) \/ RVStale(self)
                   \/ RVValid(self) \/ RVInvalid(self)
