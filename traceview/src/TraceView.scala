...
object TraceView extends JFXApp3:
  ...
  private val openSubscriptions = mutable.ListBuffer[scalafx.event.subscriptions.Subscription]()

  def makeTLAValueTreeItem(value: TLAValue, prefix: String = ""): control.TreeItem[String] =
    ...
    value match
      case leaf: (TLAValueModel | TLAValueBool | TLAValueNumber | TLAValueString) =>
      ...
      case TLAValueTuple(_value) =>
        new control.TreeItem[String]:
          ...
          children ++= _value
            ...
            .zipWithIndex
            .map: (v, idx) =>
              makeTLAValueTreeItem(v, prefix = s"[${idx + 1}] = ")
      case TLAValueFunction(_value) =>
        new control.TreeItem[String]:
          ...
          children ++= _value
            ...
            .iterator
            .map: (key, v) =>
              makeTLAValueTreeItem(v, prefix = s"[$key] = ")
      case TLAValueLambda(fn) =>
        new control.TreeItem[String](fn.toString())
  end makeTLAValueTreeItem

  def makeTraceRecordTreeItem(idx: Int, traceRecord: TraceRecord): control.TreeItem[String] =
    new control.TreeItem[String]:
      value = s"$idx -- ${traceRecord.titleString}"
      children += makeTLAValueTreeItem(traceRecord.value, prefix = "record = ")
            ...
            def updateChildren(): Unit =
              ...
              then
                (children.size until eventsShown()).foreach: idx =>
                  ...
                  if idx <= seq.size
                  then children += makeTraceRecordTreeItem(rseq.size - idx, TraceRecord(rseq(idx)))
            end updateChildren
            ...
            new control.SplitPane:
              ...
              items ++= Seq(
                ...
                new layout.VBox:
                  ...
                  object incDecButtons extends layout.HBox:
                    ...
                    children = Seq(
                      new control.Button("+1") with ButtonDisableHigh:
                        ...
                        onAction.value = _ =>
                          eventsShown.value = eventsShown() + 1,
                      ...
                      new control.Button("+5") with ButtonDisableHigh:
                        ...
                        onAction.value = _ =>
                          eventsShown.value = eventsShown() + 5,
                  ...
                  object showPossibleAltPositionsButton extends control.Button("Show positions where failed action(s) could happen"):
                  ...
                  object loadAnotherTraceButton extends control.Button("Load another trace"):
                    onAction() = evt =>
                      chooseDumpFile()
