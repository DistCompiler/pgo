...
public class TLC {
	 ...
	 * o -bound: The upper limit for sets effectively limiting the number of
	 * init states
	 ...
	 * o -fpbits num: the number of msb used by MultiFPSet to create nested
	 * FPSets.
	 * Defaults to 1
	 ...
	 * <strong>Note:</strong> This method set ups the static TLCGlobals
	 * variables
	 * 
	 * @return status of parsing: true iff parameter check was ok, false
	 *         otherwise
	 */
	...
	public boolean handleParameters(String[] args) {
			...
			if (args[index].equals("-simulate")) {
				...
				if (index < args.length) {
					try {
						...
						// a) to set the relative memory to be used for
						// fingerprints (being machine independent)
						// b) to set the absolute memory to be used for
						// fingerprints
						//
						// In order to set memory relatively, a value in the
						// domain [0.0, 1.0] is interpreted as a fraction.
						// A value in the [2, Double.MaxValue] domain allocates
						// memory absolutely.
						//
						// Independently of relative or absolute mem allocation,
						// a user cannot allocate more than JVM heap space
						// available. Conversely there is the lower hard limit
						// TLC#MinFpMemSize.
		...
		if (mainFile == null) {
			...
			if (ModelInJar.hasModel()) {
				...
				TLCGlobals.tool = true; // always run in Tool mode (to parse
										// output by Toolbox later)
				TLCGlobals.chkptDuration = 0; // never use checkpoints with
												// distributed TLC (highly
												// inefficient)
				mainFile = "MC";
			} else {
	...
	public void process() {
		...
		try {
			...
			if (isSimulate) {
				...
				// The following statement moved to Spec.processSpec by LL on 10
				// March 2011
				...
				if (TLCGlobals.DFIDMax == -1) {
					...
					mc = new DFIDModelChecker(mainFile, configFile, dumpFile, deadlock, fromChkpt, true, resolver,
							specObj);
				}
	 ...
	 * 
	 * @param resolver
	 *            a resolver for the names, if <code>null</code> is used,
	 ...
	 * 
	 * @param specObj
	 *            spec object created external SANY run
	 */
