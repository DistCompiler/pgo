...
public class TLCServer extends UnicastRemoteObject implements TLCServerRMI,
	 ...
	 * 
	 * @see
	 * tlc2.tool.distributed.TLCServerRMI#registerWorker(tlc2.tool.distributed.
	 * TLCWorkerRMI)
	 */
	...
	/**
	 * An (idempotent) method to remove a (dead) TLCServerThread from the
	 * TLCServer.
	 * 
	...
	protected void modelCheck() throws IOException, InterruptedException, NotBoundException {
		...
		if (!recovered) {
			...
			try {
				...
				if (e instanceof EvalException
					...
					// LL modified error message on 7 April 2012
					MP.printError(EC.GENERAL, "initializing the server", e); // LL
																				// changed
																				// call
																				// 7
																				// April
					...
					try {
						...
						MP.printError(EC.GENERAL, "evaluating the nested" // LL
																			// changed
																			// call
																			// 7
																			// April
		...
		// Model checking results to be collected after model checking has
		// finished
		...
		// periodically.
		synchronized (this) { // TODO convert to do/while to move initial wait
								// into loop
			...
			synchronized (this) {
				if (!done) {
					...
					MP.printMessage(EC.TLC_PROGRESS_STATS, new String[] { String.valueOf(trace.getLevelForReporting()),
							...
							String.valueOf(getNewStates()), String.valueOf(statesPerMinute),
							String.valueOf(distinctStatesPerMinute) });

					// Make the TLCServer main thread sleep for one report
					// interval
					wait(REPORT_INTERVAL);
