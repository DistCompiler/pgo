  ...
  def checkErrors(
    ...
    val expectedErrors = locally {
      ...
      parenError
        ...
        .results()
        .forEachOrdered(res => resultsBuilder += new ExpectedError(res))
      lineError
        .matcher(fileContents)
        .results()
        .forEachOrdered(res => resultsBuilder += new ExpectedError(res))
    ...
    val groupOffsets =
      (groupedErrors.keysIterator ++ groupedExpectedErrors.keysIterator).distinct.toArray
        .sortInPlace()
    ...
    groupOffsets.foreach { offset =>
      val expectedNames =
        groupedExpectedErrors.getOrElse(offset, Nil).view.map(_.name).toSet
      val actualNames =
        groupedErrors.getOrElse(offset, Nil).view.map(_.productPrefix).toSet
      ...
      fail {
        d"Mismatch between expected errors and actual errors.${
            if (unexpectedErrors.nonEmpty) {
              d"Unexpected errors present:${unexpectedErrors.view.map { err =>
                  d"${err.productPrefix}(${err.sourceLocation.maybeOffset}): ${err.description} at ${err.sourceLocation.longDescription}".indented.ensureLineBreakBefore
                }.flattenDescriptions}".ensureLineBreakBefore.indented
            } else d""
          }${
            if (missingErrors.nonEmpty) {
              d"Expected errors missing:${missingErrors.view.map { err =>
                  d"${err.name} at ${err.offset}".indented.ensureLineBreakBefore
                }.flattenDescriptions}".ensureLineBreakBefore.indented
            } else d""
          }${
            if (matchingErrors.nonEmpty) {
              d"Expected errors matched:${matchingErrors.view.map { err =>
                  d"${err.productPrefix}: ${err.description} at ${err.sourceLocation.longDescription}".indented.ensureLineBreakBefore
                }.flattenDescriptions}".ensureLineBreakBefore.indented
            } else d""
