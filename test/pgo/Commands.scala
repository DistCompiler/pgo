  ...
  def fuzzIndefinitely(
    ...
    while (!hasFailed) {
      ...
      if (!results.success) {
        hasFailed = true
        val msgText =
          s"failure caught! seed was `${results.seed}`; tree size was ${results.failedTreeSize}; failedDueToError: ${results.failedDueToError}. counter-example stored at ${results.testOut}"
        println(msgText)
        ...
        val sendFuture = mailer {
          Envelope
            .from(victimUser at victimDomain)
