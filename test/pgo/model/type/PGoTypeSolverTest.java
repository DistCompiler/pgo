...
public class PGoTypeSolverTest {
	...
	public void complexTuplePromotion() {
		...
		solver.accept(new PGoTypeConstraint(
				...
				-1));
		solver.accept(new PGoTypeConstraint(
				new PGoTypeUnrealizedTuple(new HashMap<>(Collections.singletonMap(1, PGoTypeString.getInstance()))),
				c,
				-1));
		solver.accept(new PGoTypeConstraint(a, b, -1));
	...
	public void complexTuplePromotion2() {
		...
		PGoTypeVariable c = typeGenerator.get();
		PGoTypeVariable d = typeGenerator.get();
		solver.accept(new PGoTypeConstraint(new PGoTypeSlice(d), a, -1));
		...
		solver.simplify();
		assertEquals(new HashMap<PGoTypeVariable, PGoType>() {{
			put(a, new PGoTypeSlice(PGoTypeString.getInstance()));
			put(b, new PGoTypeSlice(PGoTypeString.getInstance()));
			put(c, new PGoTypeSlice(PGoTypeString.getInstance()));
			put(d, PGoTypeString.getInstance());
		}}, solver.getMapping());
	}
	...
	public void unUnifiableTuplePromotion() {
		...
		solver.accept(new PGoTypeConstraint(
				...
				-1));
		solver.accept(new PGoTypeConstraint(
				new PGoTypeUnrealizedTuple(new HashMap<>(Collections.singletonMap(0, PGoTypeInt.getInstance()))),
				e,
				-1));
		solver.accept(new PGoTypeConstraint(a, b, -1));
		solver.accept(new PGoTypeConstraint(b, c, -1));
		solver.accept(new PGoTypeConstraint(a, e, -1));
		solver.unify();
		solver.simplify();
	}
}