...
trait TLAExpressionFuzzTestUtils {
  private abstract class TLAExpressionFuzzTestProps
      extends Properties("TLAExpression") {
    def seedStr: String
    def workDir: os.Path
    ...
    property("TLA+ expr eval (true random ASTs)") =
      forAllNoShrink[TLAExpression, Prop](trueRandomExprGen) {
        (expr: TLAExpression) =>
          val mpcalSetup =
            d"----MODULE TestBed----\n" +
          ...
          locally {
            ...
            treeSizes =
              treeSizes.updated(count, treeSizes.getOrElse(count, 0L) + 1L)
          }
          ...
          try {
            ...
            try {
              val result = os
                .proc("go", "run", "./main")
                .call(cwd = workDir, mergeErrIntoOut = true, timeout = 60000)
              ...
              case err: os.SubprocessException =>
                if (
                  err.result.out.text().startsWith("panic: TLA+ type error")
                ) {
  ...
  final case class FuzzTestingResult(
      success: Boolean,
      seed: String,
      cases: Int,
      degenerateCases: Double,
      testOut: Option[os.Path],
      result: Test.Result,
  ...
  private def genFlatASTOptions(subExprs: List[TLAExpression])(implicit
    ...
    implicit class PartialFnGenProvider(iterable: Iterable[Gen[TLAExpression]])
        extends GenProvider {
      override def genIterator: Iterator[Gen[TLAExpression]] = iterable.iterator
    }

    implicit class PartialFnIterableGenProvider(gen: Gen[TLAExpression])
        extends GenProvider {
      override def genIterator: Iterator[Gen[TLAExpression]] =
        Iterator.single(gen)
    }
    ...
    val cases: Iterator[PartialFunction[List[TLAExpression], GenProvider]] =
      Iterator(
        { case Nil =>
          for {
            num <- Gen.posNum[Int]
        ...
        },
        {
          case subExprs: List[TLAExpression]
              if subExprs.nonEmpty && env
                .exists(_.ref.arity == subExprs.size) =>
        ...
        },
        {
          case subExprs: List[TLAExpression] if subExprs.nonEmpty =>
            for {
              ...
              case ident -> expr =>
                TLARecordConstructorField(TLAIdentifier(ident), expr)
            }.toList)
        },
        {
          case subExprs: List[TLAExpression] if subExprs.nonEmpty =>
  ...
  private def genNamedASTOptions(
      breadth: Int,
      makeExpr: (
    ...
    def cleanIdentifier(implicit env: Set[ById[DefinitionOne]]): Gen[String] = {
      ...
      val envNames = env.view
        .map(_.ref.identifier.asInstanceOf[ScopeIdentifierName].name.id)
        .toSet
