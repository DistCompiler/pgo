	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.MakeTLABool(tla.TLA_EqualsSymbol(condition, iface.Self()).AsBool() && condition0.AsBool()).AsBool() {
				err = iface.Write(shouldSync, nil, tla.TLA_TRUE)
				if err != nil {
					return err
				}
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_NotEqualsSymbol(condition55, NULL(iface)).AsBool() {
				...
				case 0:
					...
					_ = value
					err = iface.Write(body, nil, tla.MakeTLARecord([]tla.TLARecordField{
					...
					err = iface.Write(req29, nil, tla.MakeTLARecord([]tla.TLARecordField{
						{tla.MakeTLAString("from"), iface.Self()},
	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_NotEqualsSymbol(condition68, NULL(iface)).AsBool() {
				...
				case 0:
					err = iface.Write(body1, nil, tla.MakeTLARecord([]tla.TLARecordField{
					...
					err = iface.Write(req32, nil, tla.MakeTLARecord([]tla.TLARecordField{
						{tla.MakeTLAString("from"), iface.Self()},
