	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_EqualsSymbol(condition11.ApplyFunction(tla.MakeTLAString("mtype")), RequestVoteRequest(iface)).AsBool() {
				...
				condition15, err = iface.Read(currentTerm2, []tla.TLAValue{i2})
				if err != nil {
					return err
				}
				...
				if tla.TLA_EqualsSymbol(condition16.ApplyFunction(tla.MakeTLAString("mtype")), RequestVoteResponse(iface)).AsBool() {
					...
					if tla.TLA_LessThanSymbol(condition19.ApplyFunction(tla.MakeTLAString("mterm")), condition20).AsBool() {
						...
						condition22, err = iface.Read(currentTerm2, []tla.TLAValue{i3})
						if err != nil {
							return err
						}
					...
					if tla.TLA_EqualsSymbol(condition24.ApplyFunction(tla.MakeTLAString("mtype")), AppendEntriesRequest(iface)).AsBool() {
						...
						condition28, err = iface.Read(currentTerm2, []tla.TLAValue{i4})
						if err != nil {
							return err
						}
						...
						condition30, err = iface.Read(currentTerm2, []tla.TLAValue{i4})
						if err != nil {
							return err
						}
						...
						condition33, err = iface.Read(currentTerm2, []tla.TLAValue{i4})
						if err != nil {
							return err
						}
						...
						condition35, err = iface.Read(currentTerm2, []tla.TLAValue{i4})
						if err != nil {
							return err
						}
						...
						if tla.MakeTLABool(tla.TLA_LessThanSymbol(condition32.ApplyFunction(tla.MakeTLAString("mterm")), condition33).AsBool() || tla.MakeTLABool(tla.MakeTLABool(tla.TLA_EqualsSymbol(condition34.ApplyFunction(tla.MakeTLAString("mterm")), condition35).AsBool() && tla.TLA_EqualsSymbol(condition36, Follower(iface)).AsBool()).AsBool() && tla.TLA_LogicalNotSymbol(logOK0).AsBool()).AsBool()).AsBool() {
							...
							condition38, err = iface.Read(currentTerm2, []tla.TLAValue{i4})
							if err != nil {
								return err
							}
						...
						if tla.TLA_EqualsSymbol(condition52.ApplyFunction(tla.MakeTLAString("mtype")), AppendEntriesResponse(iface)).AsBool() {
							...
							if tla.TLA_LessThanSymbol(condition55.ApplyFunction(tla.MakeTLAString("mterm")), condition56).AsBool() {
								...
								condition58, err = iface.Read(currentTerm2, []tla.TLAValue{i5})
								if err != nil {
									return err
								}
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_LessThanSymbol(condition70, condition71).AsBool() {
				...
				if tla.TLA_EqualsSymbol(cmd.ApplyFunction(tla.MakeTLAString("type")), Put(iface)).AsBool() {
					...
					err = iface.Write(sm, []tla.TLAValue{}, tla.TLA_DoubleAtSignSymbol(exprRead32, tla.TLA_ColonGreaterThanSymbol(cmd.ApplyFunction(tla.MakeTLAString("key")), cmd.ApplyFunction(tla.MakeTLAString("value")))))
					if err != nil {
						return err
					}
					// no statements
				...
				reqOkRead, err = iface.Read(sm, []tla.TLAValue{})
				if err != nil {
					return err
				}
				var reqOk tla.TLAValue = tla.TLA_InSymbol(cmd.ApplyFunction(tla.MakeTLAString("key")), tla.TLA_DomainSymbol(reqOkRead))
				_ = reqOk
				...
				err = iface.Write(net5, []tla.TLAValue{entry0.ApplyFunction(tla.MakeTLAString("client"))}, tla.MakeTLARecord([]tla.TLARecordField{
					...
					{tla.MakeTLAString("mresponse"), tla.MakeTLARecord([]tla.TLARecordField{
						...
						{tla.MakeTLAString("value"), func() tla.TLAValue {
							if reqOk.AsBool() {
								return exprRead33.ApplyFunction(cmd.ApplyFunction(tla.MakeTLAString("key")))
							} else {
								return Nil(iface)
							}
						}()},
			...
			if tla.MakeTLABool(tla.MakeTLABool(condition78.AsBool() && tla.TLA_EqualsSymbol(condition80, Leader(iface)).AsBool()).AsBool() && tla.TLA_LessThanOrEqualSymbol(condition81, iface.GetConstant("NumServers")()).AsBool()).AsBool() {
				...
				if tla.TLA_NotEqualsSymbol(condition82, condition83).AsBool() {
					...
					case 0:
						...
						exprRead36, err = iface.Read(currentTerm26, []tla.TLAValue{exprRead35})
						if err != nil {
							return err
						}
						...
						exprRead38, err = iface.Read(commitIndex6, []tla.TLAValue{exprRead37})
						if err != nil {
							return err
						}
						...
						exprRead40, err = iface.Read(idx8, []tla.TLAValue{})
						if err != nil {
							return err
						}
				...
				exprRead34, err = iface.Read(idx8, []tla.TLAValue{})
				if err != nil {
					return err
				}
				err = iface.Write(idx8, []tla.TLAValue{}, tla.TLA_PlusSymbol(exprRead34, tla.MakeTLANumber(1)))
				if err != nil {
					return err
				}
				return iface.Goto("AServerSender.appendEntriesLoop")
	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_TRUE.AsBool() {
				...
				exprRead41, err = iface.Read(in2, []tla.TLAValue{})
				if err != nil {
					return err
				}
				err = iface.Write(req, []tla.TLAValue{}, exprRead41)
				if err != nil {
					return err
				}
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			if tla.TLA_EqualsSymbol(condition87.ApplyFunction(tla.MakeTLAString("type")), Put(iface)).AsBool() {
				...
				case 0:
					...
					exprRead43, err = iface.Read(req0, []tla.TLAValue{})
					if err != nil {
						return err
					}
					var exprRead44 tla.TLAValue
					exprRead44, err = iface.Read(leader3, []tla.TLAValue{})
					if err != nil {
						return err
					}
					...
					err = iface.Write(net7, []tla.TLAValue{indexRead2}, tla.MakeTLARecord([]tla.TLARecordField{
						...
						{tla.MakeTLAString("mcmd"), tla.MakeTLARecord([]tla.TLARecordField{
							...
							{tla.MakeTLAString("key"), exprRead42.ApplyFunction(tla.MakeTLAString("key"))},
							{tla.MakeTLAString("value"), exprRead43.ApplyFunction(tla.MakeTLAString("value"))},
						})},
						{tla.MakeTLAString("msource"), iface.Self()},
						{tla.MakeTLAString("mdest"), exprRead44},
					}))
					if err != nil {
						return err
					}
				...
				if tla.TLA_EqualsSymbol(condition88.ApplyFunction(tla.MakeTLAString("type")), Get(iface)).AsBool() {
					...
					case 0:
						...
						exprRead45, err = iface.Read(req0, []tla.TLAValue{})
						if err != nil {
							return err
						}
						var exprRead46 tla.TLAValue
						exprRead46, err = iface.Read(leader3, []tla.TLAValue{})
						if err != nil {
							return err
						}
						...
						err = iface.Write(net7, []tla.TLAValue{indexRead3}, tla.MakeTLARecord([]tla.TLARecordField{
							...
							{tla.MakeTLAString("mcmd"), tla.MakeTLARecord([]tla.TLARecordField{
								...
								{tla.MakeTLAString("key"), exprRead45.ApplyFunction(tla.MakeTLAString("key"))},
							})},
							{tla.MakeTLAString("msource"), iface.Self()},
							{tla.MakeTLAString("mdest"), exprRead46},
						}))
						if err != nil {
							return err
						}
	...
	distsys.MPCalCriticalSection{
		...
		Body: func(iface distsys.ArchetypeInterface) error {
			...
			case 0:
				...
				exprRead47, err = iface.Read(net9, []tla.TLAValue{iface.Self()})
				if err != nil {
					return err
				}
				err = iface.Write(resp, []tla.TLAValue{}, exprRead47)
				if err != nil {
					return err
				}
				...
				err = iface.Write(out, []tla.TLAValue{}, exprRead48)
				if err != nil {
					return err
				}
				var exprRead49 tla.TLAValue
				exprRead49, err = iface.Read(resp, []tla.TLAValue{})
				if err != nil {
					return err
				}
				err = iface.Write(leader11, []tla.TLAValue{}, exprRead49.ApplyFunction(tla.MakeTLAString("mleaderHint")))
				if err != nil {
					return err
				}
				...
				if tla.TLA_LogicalNotSymbol(condition93.ApplyFunction(tla.MakeTLAString("msuccess"))).AsBool() {
					return iface.Goto("AClient.sndReq")
				} else {
					...
					condition95, err = iface.Read(req5, []tla.TLAValue{})
					if err != nil {
						return err
					}
					if tla.TLA_NotEqualsSymbol(condition94.ApplyFunction(tla.MakeTLAString("mresponse")).ApplyFunction(tla.MakeTLAString("key")), condition95.ApplyFunction(tla.MakeTLAString("key"))).AsBool() {
						return iface.Goto("AClient.rcvResp")
					} else {
			...
			case 1:
				...
				condition96, err = iface.Read(leader11, []tla.TLAValue{})
				if err != nil {
					return err
				}
				var condition97 tla.TLAValue
				condition97, err = iface.Read(fd8, []tla.TLAValue{condition96})
				if err != nil {
					return err
				}
				var condition98 tla.TLAValue
				condition98, err = iface.Read(netLen0, []tla.TLAValue{iface.Self()})
				if err != nil {
					return err
				}
				var condition99 tla.TLAValue
				condition99, err = iface.Read(timeout, []tla.TLAValue{})
				if err != nil {
					return err
				}
				if !tla.MakeTLABool(tla.MakeTLABool(condition97.AsBool() && tla.TLA_EqualsSymbol(condition98, tla.MakeTLANumber(0)).AsBool()).AsBool() || condition99.AsBool()).AsBool() {
					return distsys.ErrCriticalSectionAborted
				}
