...
struct QueueWorkloadContext: public omnilink::WorkloadContext<QueueWorkloadContext, ConcurrentQueue::AnyOperation> {
    static constexpr size_t size_limit = 16;
    // Config with deliberately small numbers to help reach edge cases within model checking limits.
    ...
    struct RunnerDefns: public WorkloadContext::RunnerDefnsBase<RunnerDefns> {
        ...
        int32_t rand_bulk_size() { 
            return std::uniform_int_distribution<int32_t>(0, max_bulk_size)(this->rand);
        }
        int32_t rand_element() {
            // Here we ensure elements enqueued by a given thread are never equal to another's
            // Not necessary strictly, but it makes validation less ambiguous
            size_t thread_num = thread_idx + 1;
            return std::uniform_int_distribution<int32_t>(thread_num * 10, (thread_num + 1) * 10 - 1)(this->rand);
        }
        ...
        void perform_operation(Ctx<ConcurrentQueue::EnqueueWithToken>& ctx) {
            size_t size = rand_bulk_size();
            // Set total_elems optimistically, and decrement it only if things go badly.
            size_t total_elems_seen = workload_context.total_elems.fetch_add(size) + size;
