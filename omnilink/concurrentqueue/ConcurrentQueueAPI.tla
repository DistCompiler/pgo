...
VARIABLES queue, enqueued, dequeued, producerTokens, consumerTokens

vars == <<queue, enqueued, dequeued, producerTokens, consumerTokens>>
...
varsEnqueued == <<dequeued, producerTokens, consumerTokens>>
varsDequeued == <<enqueued, producerTokens, consumerTokens>>
varsEnqueueFail == <<queue, enqueued>>
varsDequeueFail == <<queue, dequeued>>
varsCreateProducer == <<queue, enqueued, dequeued, consumerTokens>>
varsCreateConsumer == <<queue, enqueued, dequeued, producerTokens>>
...
QueueTryDequeueFromProducer(prodToken, element, success) ==
    ...
    /\ IF Len(queue) > 0
       THEN /\ success = TRUE
            ...
            /\ dequeued' = dequeued + 1
       ELSE /\ success = FALSE
            /\ element \in Elements  \* Dummy value when queue is empty
            /\ UNCHANGED varsDequeueFail
    /\ UNCHANGED varsDequeued
