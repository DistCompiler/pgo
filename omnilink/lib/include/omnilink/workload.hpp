...
struct RunnerDefns<_Self, _WorkloadContext, std::variant<Operations...>> {
  ...
  RunnerDefns(WorkloadContext &_workload_context, std::size_t _thread_idx)
      : workload_context{_workload_context}, thread_idx{_thread_idx} {}

  RunnerDefns(const Self &) = delete;
  RunnerDefns(Self &&) = delete;

#ifdef OMNILINK_USE_RANDOM_DEVICE
  ...
  template <typename Operation> struct Ctx {
    Operation &op;
    void refine_op_start() { log::mark_operation_start(); }
    void refine_op_end() { log::mark_operation_end(); }
    void unsupported() { throw omnilink::UnsupportedException{}; }
    friend struct RunnerDefns<_Self, _WorkloadContext,
                              std::variant<Operations...>>;

  private:
    Ctx(_WorkloadContext &w)
        : w{w}, op{log::template start_operation<Operation>()} {}
    _WorkloadContext &w;
  };
  ...
  void operator()() {
    ...
    auto uniform_idx_dist =
        std::uniform_int_distribution<std::size_t>(0, operations.size() - 1);

    for (std::size_t i = 0; i < workload_context.operation_count; ++i) {
      std::array<uint64_t, sizeof...(Operations)> consecutive_failures = {0};
      bool shouldTry = true;
      while (shouldTry) {
        auto min_consecutive_fail_it = std::min_element(
            std::begin(consecutive_failures), std::end(consecutive_failures));
        if (*min_consecutive_fail_it >=
            workload_context.max_consecutive_failures) {
          std::cerr << "[thread " << thread_idx << "] "
                    << "could not choose a valid operation after "
                    << workload_context.max_consecutive_failures
                    << " attempts! Exiting early at iteration " << i
                    << " rather than risk an infinite loop." << std::endl;
...
private:
  ...
  struct TerminateThreadSentinel {
    // When reaching this destructor, we know all subclass fields have been
    // cleaned up. Therefore, we have finished any cleanup ops our workload
    // requires.
  ...
  constexpr _Self &self() { return static_cast<_Self &>(*this); }
...
template <typename _Self, typename AnyOperation> struct WorkloadContext {
  ...
  const std::size_t max_consecutive_failures =
      env_opt("OMNILINK_CONSECUTIVE_FAILURES", 10000);
  ...
  void run() {
    ...
    for (std::size_t i = 0; i < thread_count; ++i) {
      threads.emplace_back([this, i]() {
        ...
        // Otherwise, if it uses any thread-local storage, you will see
        // confusing and impossible-seeming behavior.
...
private:
  ...
  std::size_t env_opt(const char *opt_name,
                      std::optional<std::size_t> _default = {}) {
    ...
    if (val) {
      ...
      out << "Couldn't guess thread count because " << opt_name
          << " wasn't set";
  ...
  void install_crash_handler() {
    assert(std::signal(SIGSEGV, omnilink::crash_handler) != SIG_ERR);
    assert(std::signal(SIGFPE, omnilink::crash_handler) != SIG_ERR);
    assert(std::signal(SIGILL, omnilink::crash_handler) != SIG_ERR);
  }
};
