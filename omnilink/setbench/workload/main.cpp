...
struct TreeWorkloadContext
    : public omnilink::WorkloadContext<TreeWorkloadContext,
                                       SetBench::AnyOperation> {
  DataStructure data_structure{
      int(this->thread_count +
          1),  // +1 because this constructor (main thread) counts!
      12,      // max key
      -1,      // ignored
  ...
  struct RunnerDefns : public WorkloadContext::RunnerDefnsBase<RunnerDefns> {
    ...
    void perform_operation(Ctx<SetBench::KVInsertIfAbsent> &ctx) {
      ...
      int32_t key = rand_kv();
      int32_t value = rand_kv();
      int32_t result = workload_context.data_structure.insertIfAbsent(
    ...
    void perform_operation(Ctx<SetBench::KVRangeQuery> &ctx) {
      ...
      int count = workload_context.data_structure.rangeQuery(thread_idx, lo, hi,
                                                             nullptr, nullptr);
      ctx.op.lo = lo;
      ctx.op.hi = hi;
      ctx.op.count = count;
...
int main() { return TreeWorkloadContext::main(); }
